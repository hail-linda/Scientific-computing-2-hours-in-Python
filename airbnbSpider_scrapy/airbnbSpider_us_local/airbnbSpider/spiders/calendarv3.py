...
class calenderSpider(Spider):
    ...
    def start_requests(self): 
        f = open("/opt/houseId30K.txt", 'r')
        houseList = json.loads(f.readline())
        # print(houseList)
        for count in range(len(houseList)):
            if count % 1500 == 0 and not count == 0:
                print("wait")
                time.sleep(5)
            house_id = houseList[count]
            with open("/opt/timelog.txt", 'a+') as timelogF :
                now = int(round(time.time()*1000))
                print(count*100.0/len(houseList),"%\t",house_id,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(now/1000)),file=timelogF)
