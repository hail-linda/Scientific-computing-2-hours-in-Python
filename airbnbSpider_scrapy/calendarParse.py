...
class calendarParse():
    ...
    def dbInsert(self):
        self.vals = []
        self.sql = "INSERT IGNORE INTO `airbnbspider`.`order` (`house_id`, `fetch_date`, `order_date`, `hash`)\
                 VALUES (%s,%s,%s,%s);"
        ...
        self.vals = []
        self.sql = "INSERT IGNORE INTO `airbnbspider`.`price` (`house_id`, `fetch_date`, `order_date`,`price`, `hash`)\
                 VALUES (%s,%s,%s,%s,%s);"
...
if __name__ == "__main__":
    ...
    db = pymysql.connect(
        "localhost", "root", "delta=b2-4ac", "airbnbspider")
    cursor = db.cursor()
    sql = "SELECT id FROM `airbnbspider`.`calendarresponse` order by id desc limit 20"
    cursor.execute(sql)
    db.commit()
