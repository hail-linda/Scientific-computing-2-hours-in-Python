...
def sql_listingMonthlyStatistics(listingId,statDate):
    ...
    FROM (
        SELECT `lastprice`.*
          FROM(
                SELECT `order`.`house_id`, `order`.`order_date`
                  FROM `order`
                  LEFT JOIN `cancel` ON `order`.`order_date`= `cancel`.`order_date`
                   AND `order`.`house_id`= `cancel`.`house_id`
                 WHERE `order`.`house_id`= '{}'
                   AND YEAR(`order`.`order_date`)= YEAR('{}')
                   AND MONTH(`order`.`order_date`)= month('{}')
                   AND `cancel`.`id` IS NULL
                 ORDER BY `order`.`order_date` ASC) AS `orderwhitoutcancel`
          LEFT JOIN(
                SELECT `price` .`house_id`, `price` .`fetch_date`, `lastprice`.`order_date`, `price` .`price`
                  FROM(
                SELECT MAX(`id`) as `id`, MAX(`order_date`)  as `order_date`
                  FROM `price`
                 WHERE `house_id`= '{}' AND `fetch_date` >= '2021-02-20'
                 group by `order_date`)  AS `lastprice`
                  LEFT JOIN `price` ON `lastprice`.`id`= `price` .`id`) AS `lastprice` ON `orderwhitoutcancel`.`house_id`= `lastprice` .`house_id`
...
def sql_listingPriceMonthlyStatistics(listingId,statDate):
    ...
    FROM(
            ...
            FROM(
                    ...
                    group by `order_date`)  AS `lastprice`
    LEFT JOIN `price`
        ON `lastprice`.`id`= `price` .`id`
    WHERE YEAR(`price`.`order_date`)= YEAR('{}')
    AND MONTH(`price`.`order_date`)= month('{}')
...
def areaMonthlyStatistics(areaRow,statDate,dimension):
    ...
    airbnbRating = {"reviewCount":0,"reviewSummary_wei_zhi_bian_li":0,"reviewSummary_ru_zhu_bian_jie":0,"reviewSummary_ru_shi_miao_shu":0,"reviewSummary_gan_jing_wei_sheng":0,"reviewSummary_gou_tong_shun_chang":0,"reviewSummary_gao_xing_jia_bi":0}
    rentalSize = {}
    amenity = {}
    # rentalSize init
    ...
    for item in ["reviewSummary_wei_zhi_bian_li","reviewSummary_ru_zhu_bian_jie","reviewSummary_ru_shi_miao_shu","reviewSummary_gan_jing_wei_sheng","reviewSummary_gou_tong_shun_chang","reviewSummary_gao_xing_jia_bi"]:
        if airbnbRating["reviewCount"] > 0:
          airbnbRating[item] =airbnbRating[item]/airbnbRating["reviewCount"]
...
def listingMonthlyStatistics(listingId,statDate):
    ...
    sm.release()
    sql = """
    INSERT INTO `airbnb_scrapy`.`monthlystat`
